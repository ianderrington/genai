...
def get_openai_cost_details(api_key, start_date, end_date, org_id=None):
    ...
    response = requests.get(
        "https://api.openai.com/dashboard/billing/usage", params=params, headers=headers
    )
...
def get_openai_subscription_details(api_key, org_id=None):
    ...
    if org_id:
        headers["openai-organization"] = f"org-{org_id}"
    response = requests.get(
        "https://api.openai.com/dashboard/billing/subscription", headers=headers
    )
    return response.json()
...
OPENAI_API_KEY = os.environ[
    "OPENAI_API_KEY"
]  # set the one for which you want to calculate cost
