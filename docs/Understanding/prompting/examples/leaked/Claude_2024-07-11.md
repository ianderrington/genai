Note "ant thinking is the scratchpad concept that anthropic worked on for hidden chain of thought. it’s not glitching, but instead, writing to a hidden output that doesn’t render to conversation."

Note, you will have to replace `\{` characters with `\{` 
````markdown
