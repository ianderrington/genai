...
class ShareableAdmonitionPlugin(BasePlugin):
    ...
    def on_page_markdown(self, markdown_content, page, config, files):
        ...
        for line in markdown_content.splitlines():
            if self.RE.match(line):
                if in_admonition:
                    # Process the previous admonition block before starting a new one
                    self.process_admonition_block(new_content, admonition_block, share_name, title, page, config)
                in_admonition = True
                admonition_type, tag, title, share_name = self.RE.match(line).groups()
                admonition_block = []
                continue
            ...
            if in_admonition:
                if line.startswith('    ') or line.startswith('\t'):  # Content of the admonition
                    admonition_block.append(line.lstrip())  # Remove left indentation
                else:
                    # Process the admonition block
                    self.process_admonition_block(new_content, admonition_block, share_name, title, page, config)
                    in_admonition = False
                    new_content.append(line)  # Include the current line as it's outside the admonition
            else:
        ...
        if in_admonition:
            # Process the last admonition block in the file
            self.process_admonition_block(new_content, admonition_block, share_name, title, page, config)

        return '\n'.join(new_content)

    def process_admonition_block(self, new_content, admonition_block, share_name, title, page, config):
        # Process extracted admonition block
        admonition_content = '\n'.join(admonition_block)
        admonition_block_content = markdown.markdown(admonition_content, extensions=config['markdown_extensions'])
