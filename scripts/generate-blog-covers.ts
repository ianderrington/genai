...
rather than a simplified icon. Restrained, sophisticated color washes
(indigo, rust, sage, ochre) layered over the line art. One focal element
carries a soft warm radiant glow or backlight, and a single continuous warm
color wash bleeds across the whole background — atmospheric and inviting,
bright and hopeful, not dark or heavy.
`.trim();

function composePrompt(subject: string, noPerson: boolean): string {
...
async function generateImage(subject: string, noPerson: boolean): Promise<Buffer> {
  const result = await aiGenerateImage({
    model: openai.image('gpt-image-1'),
    prompt: composePrompt(subject, noPerson),
    size: '1536x1024',
...
async function main() {
  ...
  console.log(`Found ${candidates.length} posts without a cover; generating ${targets.length}.\n`);
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      const bytes = await generateImage(subject, noPerson);
